...
public enum SJTabBarItemPositioning : Int {
...
internal protocol SJTabBarDelegate: NSObjectProtocol {
...
class SJTabBar: UITabBar {
    ...
    internal weak var customDelegate: SJTabBarDelegate?
    ...
    public var itemCustomPositioning: SJTabBarItemPositioning? {
        didSet {
...
//            ESTabBarController.printError("empty items")
        ...
        for (idx, item) in tabBarItems.enumerated() {
            let container = SJTabbarItemContainer.init(self, tag: 1000 + idx)
            self.addSubview(container)
            self.containers.append(container)
            
            if let item = item as? SJTabBarItem, let contentView = item.contentView {
...
//        self.updateAccessibilityLabels()
    ...
    @objc func highlightAction(_ sender: AnyObject?) {
        guard let container = sender as? SJTabbarItemContainer else {
            return
        }
